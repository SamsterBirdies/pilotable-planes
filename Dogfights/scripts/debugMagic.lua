...
if preG == nil then
  ...
  local getStacktrace = function(stacktrace, pCallFunctionName, pCallParameters, pCallResult)
    local extractFunctionPosition = function(line)
      ...
      return {
        fileName = fileNameComment:sub(1, 2) == '--' and fileNameComment:sub(3) or "(file starting with '"..line:sub(10, s-1).."')",
      ...
      if lineIdx > 2 then
        ...
        if lineType == 0 then
          ...
          if pos then
            ...
            table.insert(stacktraceTable, {functionName = functionName, fileName = position.fileName, lineNo = position.lineNo, parameter = parameter })
          end
        end
        if lineType == 1 then
        ...
        if ok then
          ...
          local o, r = pcall(function(result, functionName, ...)
            ...
            for _, d in ipairs(stacktrace) do
              errorLine = errorLine .. "\r\n  at " .. d.fileName .. ':' .. d.lineNo ..(d.functionName and ':'..d.functionName..'('..(d.parameter or '')..')' or '')
            end
            Log(errorLine)
