...
function UpdatePlanePhysics(id, saveName, teamId)
	...
	--test if plane is under map extents
	if NodePosition(id).y < max_height then return end
	--velocity
...
function UpdateHeliPhysics(id, saveName, teamId)
	...
	--do wind
	if data.heli_effect and data.do_heli_effect and GetProjectileParamBool(saveName, teamId, "sb_planes.heli_effect", true) then
